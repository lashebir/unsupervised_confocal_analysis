...
#         # reader = aicsimageio.AICSImage(file_path, reader=LifReader)
...
def convert_max_proj_tensor(file_path, ch_order):
    ...
    try:
        ...
        img = lif.get_image(0)  # get first image in the file
        ...
        for ch in range(img.channels):
            ...
            # Build z-stack for this channel
            z_slices = []
            for z in range(img.dims.z):
                frame = np.array(img.get_frame(z=z, t=0, c=ch))
                z_slices.append(frame)
            channel_data = np.stack(z_slices, axis=0)  # (Z, H, W)
...
print(torch.backends.mps.is_available() if hasattr(torch.backends, 'mps') else "no mps backend")
...
image_dir = Path(r'D:\Leah\Liberman Data\Confocal Data Charles Liberman')
image_paths = collect_all_lif_paths(image_dir)
...
# [repr(x) for x in image_paths[:20]]
# repr(image_paths[:5])  # Show first 5 paths to verify
print(str(image_dir))          # O:\BPHO Dropbox\path
print(os.path.exists(image_dir))  # should be True
...
class ShallowCNN_Tuning:
    ...
    def build(self, device='cuda'):
        ...
        Returns:
            model: ShallowCNN instance
            paired_dataset: SynapseImageDataset for training
    ...
    def train(self, model, dataset, epochs=1000, device='cuda'):
        """
...
# # lif_file = LifFile(r'D:\Leah\unsupervised_clustering\unsupervised_confocal_analysis\WPZ104L.CtBP2.GluR2.NF.Myo7.IHC.11.3.v1.lif')
# # print(len(lif_file.image_list))

# o_file = LifFile(r'O:\BPHO Dropbox\Manor Lab\ManorLab\File requests\liberman_lab_data\Paired_Ephys_SynapseConfocal\Confocal Data Charles Liberman\WPZ104\WPZ104L.CtBP2.GluR2.NF.Myo7.IHC.11.3.v1.lif')
# print(len(o_file.image_list))

# # Quick sanity check on first few files
# for p in image_paths[:3]:
#     print(repr(p))
#     t = convert_max_proj_tensor(p, ['myo7', 'glur2', 'ctbp2', 'nf'])
#     if t is not None:
#         print(f"  Shape: {t.shape}")

# print(repr(image_paths[0])) #get path for hardcoding

# file that works:
# O:\BPHO Dropbox\Manor Lab\ManorLab\File requests\liberman_lab_data\Paired_Ephys_SynapseConfocal\Confocal Data Charles Liberman\WPZ104\WPZ104L.CtBP2.GluR2.NF.Myo7.IHC.11.3.v1.lif
# testing = LifFile(r'O:\BPHO Dropbox\Manor Lab\ManorLab\File requests\liberman_lab_data\Paired_Ephys_SynapseConfocal\Confocal Data Charles Liberman\WPZ104\WPZ104L.CtBP2.GluR2.NF.Myo7.IHC.11.3.v1.lif')

# failure files
# testing = LifFile(r'O:\BPHO Dropbox\Manor Lab\ManorLab\File requests\liberman_lab_data\Paired_Ephys_SynapseConfocal\Confocal Data Charles Liberman\WPZ100\WPZ100L.CtBP2.GluR2.NF.Myo7.IHC.11.3.v1.lif')

working = r'O:\BPHO Dropbox\Manor Lab\ManorLab\File requests\liberman_lab_data\Paired_Ephys_SynapseConfocal\Confocal Data Charles Liberman\WPZ104\WPZ104L.CtBP2.GluR2.NF.Myo7.IHC.11.3.v1.lif'
failing = image_paths[0]

import os
print(os.path.basename(failing))
print(os.path.basename(working))

# print(working.encode())
# print(failing.encode())

# print(f"Working size: {os.path.getsize(working)} bytes")
# print(f"Failing size: {os.path.getsize(failing)} bytes")
# # print('\n\n',list(failing.encode()))

# bytes_list = list(failing.encode())
# for i, b in enumerate(bytes_list):
#     if b > 127:
#         print(f"Position {i}: byte {b}, char '{failing[i]}'")

# # Check if the failing file can be opened at all
# with open(working, 'rb') as f:
#     header = f.read(100)
#     print(f"Working header: {header[:20]}")
#     header = f.read(20)
#     print(f"Working first byte: {hex(header[0])}")
...
# f = io.FileIO(failing, 'rb')
# header = f.read(20)
# print(header[:20])
# f.close()
# # with open(failing, 'rb') as f:
# #     header = f.read(100)
# #     print(f"Failing header: {header[:20]}")
# #     header = f.read(20)
# #     print(f"Failing first byte: {hex(header[0])}")




# testing = LifFile(image_paths[0])
# import readlif
# print(readlif.__version__)
# print(readlif.__file__)

# b'O:\\BPHO Dropbox\\Manor Lab\\ManorLab\\File requests\\liberman_lab_data\\Paired_Ephys_SynapseConfocal\\Confocal Data Charles Liberman\\WPZ104\\WPZ104L.CtBP2.GluR2.NF.Myo7.IHC.11.3.v1.lif'
# b'O:\\BPHO Dropbox\\Manor Lab\\ManorLab\\File requests\\liberman_lab_data\\Paired_Ephys_SynapseConfocal\\Confocal Data Charles Liberman\\WPZ100\\WPZ100L.CtBP2.GluR2.NF.Myo7.IHC.11.3.v1.lif'


# In[ ]:


todays_ft = f'3_20' # update this per ft round
...
print(f'Best Shallow CNN Config Based on Davies-Bouldin Score: {best_config[1], best_config[2]}')
